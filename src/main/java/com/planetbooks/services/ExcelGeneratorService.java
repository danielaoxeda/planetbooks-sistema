...
public class ExcelGeneratorService {
    ...
    public ByteArrayInputStream generateBooksExcel(List<BookFilterDTO> rows) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {

            Sheet sheet = workbook.createSheet("Books Report");

            String[] headers = { "#", "Title", "Exam", "Price (USD)", "Publisher" };
            Row headerRow = sheet.createRow(0);
            CellStyle headerStyle = workbook.createCellStyle();
            Font headerFont = workbook.createFont();
            headerFont.setBold(true);
            ...
            int rowIdx = 1;
            for (BookFilterDTO b : rows) {
                Row row = sheet.createRow(rowIdx++);
                row.createCell(0).setCellValue(rowIdx - 1);
                row.createCell(1).setCellValue(b.getTitle());
                row.createCell(2).setCellValue(b.getExam());
                row.createCell(3).setCellValue(b.getPrice());
